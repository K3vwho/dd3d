...
def main(cfg):
    ...
    if cfg.EVAL_ONLY:
        ...
        if cfg.TEST.AUG.ENABLED:
            test_results.update(
                do_test(cfg, model, is_last=True, use_tta=True))
    ...
    if comm.is_distributed():
        ...
        model = SyncBatchNorm.convert_sync_batchnorm(
            model, process_group=sync_bn_pg)
...
def do_train(cfg, model):
    ...
    writers = [CommonMetricPrinter(
        max_iter)] if d2_comm.is_main_process() else []

    train_mapper = get_dataset_mapper(cfg, is_train=True)
    dataloader, dataset_dicts = build_train_dataloader(
        cfg, mapper=train_mapper)
    ...
    for data, iteration in zip(dataloader, range(max_iter * accumulate_grad_batches)):
        ...
        loss_dict = {name: loss / accumulate_grad_batches for name,
                     loss in loss_dict.items()}
        ...
        loss_dict_reduced = {k: v.item()
                             for k, v in d2_comm.reduce_dict(loss_dict).items()}
        assert torch.isfinite(torch.as_tensor(
            list(loss_dict_reduced.values()))).all(), loss_dict_reduced
        ...
        if batch_iter > 5 and (batch_iter % 20 == 0 or batch_iter == max_iter):
            ...
            if d2_comm.is_main_process() and cfg.WANDB.ENABLED:
                wandb.log({"epoch": 1 + num_images_seen //
                          len(dataset_dicts)}, step=batch_iter)
                wandb.log({"num_images_seen": num_images_seen},
                          step=batch_iter)
        ...
        if cfg.VIS.DATALOADER_ENABLED and batch_iter % cfg.VIS.DATALOADER_PERIOD == 0 and d2_comm.is_main_process():
            ...
            visualizer_names = MetadataCatalog.get(
                dataset_name).loader_visualizers
            ...
            if cfg.WANDB.ENABLED:
                per_image_vis = [mosaic(list(viz_images[idx].values()))
                                 for idx in range(len(data))]
                wandb.log({
                    "dataloader": [wandb.Image(vis, caption=f"idx={idx}") for idx, vis in enumerate(per_image_vis)]
                },
                    step=batch_iter)
            save_vis(viz_images, os.path.join(
                os.getcwd(), "visualization"), "dataloader", step=batch_iter)

        if d2_comm.is_main_process():
            # (fvcore) model_0004999.pth checkpoints 5000-th iteration
            periodic_checkpointer.step(batch_iter - 1)
